...
export async function register(registration) {
  let formData = new FormData();
  for (let i in registration) formData.append(i, registration[i]);
...
export async function getArticle(article) {
  return await fetch(
    "http://www.ruck.eu/backend/rest/products/fpn/" +
      article +
      "?frequency=50&api_key=public",
    {
      method: "GET",
      headers: {
...
export async function getArticle_2(response) {
  return await fetch(
    "http://www.ruck.eu/backend/rest/calculations/standarddata?partNumber=" +
      response +
      "&curveType=0&etaDistance=70&curvePoints=50&language=de&api_key=public",
    {
      method: "GET"
    }
  ).then(response => response.json());
...
export async function getArticle_3() {
  return await fetch(
    "http://www.ruck.eu//backend/rest/calculations/dutydata?partNumber=132861_50_230_0_Us&qV=2&psF=528&FunctionType=0&FunctionPoints=50&api_key=public",
    {
      method: "GET"
    }
  ).then(response => response.json());
...
export async function shopAdd(shopAdd) {
  let formData = new FormData();
  for (let i in shopAdd) formData.append(i, shopAdd[i]);
