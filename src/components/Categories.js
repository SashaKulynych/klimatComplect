...
class Categories extends Component {
  ...
  async componentDidMount() {
    ...
    await API.getSubCategories(this.props.match.params.id).then(value => {
      ...
      this.setState({
        categories: value,
        showCategory: value.length !== 0 ? value[0].id : 0
      });
    });
    ...
    let categories = this.state.categories.map((value, index) => {
      return (
        <li key={index} onClick={() => this.changeTab(index, value)}>
          <span
            className={
              this.state.tab === index ? css(styles.borderYellow) : null
            }
          >
            {value.name}
          </span>
      ...
      return (
        <a href={a}>
          <li key={index}>
            ...
            <div className="itemBottom">{value.name}</div>
          </li>
        </a>
      );
    });
